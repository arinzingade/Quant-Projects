...
import redis
from helpers import place_order, get_current_price, get_open_orders, cancel_all_orders, fetch_positions, close_all_positions, custom_round_to_10_not_5
...
def place_bracket_limit_orders(account_number, symbol, qty, upper_pct, lower_pct, side):
    ...
    if side == 'NEUTRAL':
        ...
        order_1_info = place_order(account_number, symbol, upper_limit_price, 'LIMIT', qty, 'SELL', False)
        order_2_info = place_order(account_number, symbol, lower_limit_price, 'STOP_LIMIT', qty, 'SELL', False)
    
    elif side == 'SELL':
        order_1_info = place_order(account_number, symbol, upper_limit_price, 'STOP_LIMIT', qty, 'BUY', False)
        order_2_info = place_order(account_number, symbol, lower_limit_price, 'LIMIT', qty, 'BUY', False)
